          ...
          <v-container>
            ...
            <v-row v-if="deliveryType=='Kurier'&&sale">
              ...
              <v-col cols="12" md="4">
                 ...
                 :headers="addressHeaders" :rows="customerAddresses" ></ModalChoose>
              </v-col>
              <v-col cols="13" md="5" >
                 <ModalInput @submit="submitAddress" title="Dodaj nowy adres" btnText="Dodaj nowy adres"
                 :headers="addressHeaders" :rows="customerAddresses" ></ModalInput>
...
export default {
  ...
  computed: {
    ...
    autoStore () {
      if (this.store !== null && typeof this.store !== 'undefined' && typeof this.store.name !== 'undefined') {
        return this.store
      } else {
        return { name: '---' }
      }       
    ...
    customerAddresses () {
      return this.addresses.filter(el => {
        if (this.customer != null) { return el.customerID == this.customer.id }
        return false
      })
    },
    autoAddress () {
  ...
  watch: {
    store: function (newStore, oldStore) {
      if (this.address == null) { this.addresses = addresses.find(el => el.id = newStore.addressID) }
    }
  },
  methods: {
